...
class Integer:
    def __init__(self, number: str):
        # Инициализация целого числа из строки        if number[0] == '-':
            # Если число отрицательное, сохраняем цифры и устанавливаем знак 1            self.number = list(map(int, number[1:]))
            self.sign = 1        else:
            # Если число положительное, сохраняем все цифры и устанавливаем знак 0            self.number = list(map(int, number[0:]))
            self.sign = 0        # Если все цифры нули, устанавливаем знак 0 (положительное)        if all(digit == 0 for digit in self.number):
            self.sign = 0    def __str__(self):
        # Строковое представление числа        if self.sign == 1 and not all(digit == 0 for digit in self.number):
            sign_prefix = '-'  # Добавляем минус для отрицательных чисел        else:
            sign_prefix = ''        number_str = ''.join(map(str, self.number))
        return sign_prefix + number_str

    def ABS_Z_N(self):
        # Возвращает модуль числа как натуральное число        return Natural(''.join(map(str, self.number)))

    def POZ_Z_D(self):
        # Определение знака числа: 0 - ноль, 1 - отрицательное, 2 - положительное        if all(digit == 0 for digit in self.number):
            return 0        elif self.sign == 1:
            return 1        else:
            return 2    def MUL_ZM_Z(self):
        # Умножение числа на -1 (смена знака)        if all(digit == 0 for digit in self.number):
            return Integer('0')  # Ноль остается нулем        result = Integer(str(self))
        result.sign = 1 - result.sign  # Меняем знак: 0->1, 1->0        return result

    @staticmethod    def TRANS_N_Z(number: Natural):
        # Преобразование натурального числа в целое        result = Integer(str(number))
        return result

    def TRANS_Z_N(self):
        # Преобразование целого числа в натуральное (только для неотрицательных)        if self.sign == 1:
            raise ValueError("Отрицательное число нельзя преобразовать в натуральное")
        return Natural(''.join(map(str, self.number)))


    # Сложение целых чисел    def ADD_ZZ_Z(self, other):
        # Если знаки одинаковые - складываем модули и сохраняем знак        if self.POZ_Z_D() == other.POZ_Z_D():
            abs_sum = self.ABS_Z_N().ADD_NN_N(other.ABS_Z_N())
            result = Integer.TRANS_N_Z(abs_sum)
            if self.POZ_Z_D() == 1:  # Оба отрицательные                result.sign = 1            return result

        # Если знаки разные - вычитаем модули        abs_self = self.ABS_Z_N()
        abs_other = other.ABS_Z_N()

        comparison = abs_self.COM_NN_D(abs_other)

        if comparison == 2:  # |self| > |other|            abs_result = abs_self.SUB_NN_N(abs_other)
            result = Integer.TRANS_N_Z(abs_result)
            result.sign = self.sign
        elif comparison == 1:  # |self| < |other|            abs_result = abs_other.SUB_NN_N(abs_self)
            result = Integer.TRANS_N_Z(abs_result)
            result.sign = other.sign
        else:  # |self| == |other|            return Integer('0')

        return result

    # Вычитание целых чисел: self - other    def SUB_ZZ_Z(self, other):
        # Преобразуем вычитание в сложение: a - b = a + (-b)        neg_other = other.MUL_ZM_Z()
        return self.ADD_ZZ_Z(neg_other)

    # Умножение целых чисел    def MUL_ZZ_Z(self, other):
        # Умножаем модули        abs_product = self.ABS_Z_N().MUL_NN_N(other.ABS_Z_N())
        result = Integer.TRANS_N_Z(abs_product)

        # Определяем знак результата        if self.POZ_Z_D() == 0 or other.POZ_Z_D() == 0:  # Если один из множителей ноль            result.sign = 0        elif self.POZ_Z_D() == other.POZ_Z_D():  # Одинаковые знаки            result.sign = 0  # Положительный        else:  # Разные знаки            result.sign = 1  # Отрицательный        return result

    # Частное от деления целого на целое (делитель отличен от нуля)    def DIV_ZZ_Z(self, other):
        if other.POZ_Z_D() == 0:
            raise ValueError("Деление на ноль невозможно")

        # Делим модули        abs_dividend = self.ABS_Z_N()
        abs_divisor = other.ABS_Z_N()

        # Проверяем, что делимое >= делителя по модулю        if abs_dividend.COM_NN_D(abs_divisor) == 1:
            quotient = Natural('0')
        else:
        ...
        # Определяем знак результата        if self.POZ_Z_D() == 0:  # Делимое равно нулю            result.sign = 0        elif self.POZ_Z_D() == other.POZ_Z_D():  # Одинаковые знаки            result.sign = 0  # Положительный        else:  # Разные знаки            result.sign = 1  # Отрицательный        return result

    # Остаток от деления целого на целое (делитель отличен от нуля)    def MOD_ZZ_Z(self, other):
        if other.POZ_Z_D() == 0:
            raise ValueError("Деление на ноль невозможно")

        # Вычисляем частное        quotient = self.DIV_ZZ_Z(other)
        # Вычисляем произведение частного и делителя        product = quotient.MUL_ZZ_Z(other)
        # Вычисляем остаток: self - product        remainder = self.SUB_ZZ_Z(product)
...
def tests_for_new_functions():
    ...
    # Тест: (a + b) * c = a*c + b*c    a = Integer("10")
    b = Integer("-5")
    c = Integer("3")
